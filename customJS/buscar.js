...
{
	$("#lblHeader_NomModulo").text("Buscar Transformadores");
			...
			if (data.Error == "")
			{
				...
				} else
				{
					...
					*/
					...
					{
						tds += '<tr><td><button type="button" class="btn btn-warning btn-icon waves-effect waves-circle waves-float btnBuscar_ItemSeleccionado" data-row-id="' + val.id + '"><span class="zmdi zmdi-arrow-forward"></span></button></td>';
					...
					$("#tblBuscar_Resultados thead tr").remove();
					$("#tblBuscar_Resultados thead").append(ths);
					$("#tblBuscar_Resultados").crearDataTable(tds);
	...
	$(document).delegate('.btnBuscar_ItemSeleccionado', 'click', function(event) {
		//var idTrafo = $(this).attr('data-row-id');
